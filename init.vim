" Editor settings
source $HOME/.config/nvim/general/settings.vim
source $HOME/.config/nvim/keys/mappings.vim

"Plugins and their configs
source $HOME/.config/nvim/plugins-include/plugins.vim
source $HOME/.config/nvim/plugins-config/treesitter.vim
source $HOME/.config/nvim/plugins-config/coc.vim
source $HOME/.config/nvim/plugins-config/vim-cmake.vim
source $HOME/.config/nvim/plugins-config/ntree.vim
source $HOME/.config/nvim/plugins-config/snips.vim
source $HOME/.config/nvim/plugins-config/telescope.vim
source $HOME/.config/nvim/plugins-config/theme.vim
source $HOME/.config/nvim/plugins-config/vimspector.vim

